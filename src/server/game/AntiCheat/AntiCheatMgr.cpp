...
void AntiCheatMgr::HackDetection(MovementInfo movementInfo, uint32 opcode)
{
        ...
        if (_deltaX + _deltaY > 0 && _deltaX + _deltaY < 50.0f)
        {
            ...
            else
            {
                ...
                if (sWorld->getBoolConfig(CONFIG_ANTICHEAT_ANTISWIM_HACK))
                    CheckForAntiswimHack(movementInfo);
            }
        }
    ...
    // DEBUG
    //player->Say("Time Diff: " + std::to_string(_timeDiff) + " ms", LANG_UNIVERSAL);
    ...
    if (_timeDiff >= _resetTimer)
    {
        ...
        // DEBUG
        //player->Say("_hackCounter reset!", LANG_UNIVERSAL);
    }
    ...
    _lastOpcode = opcode;
    _lastLiquidStatus = player->GetLiquidStatus();
...
void AntiCheatMgr::CheckForSpeedHack(MovementInfo movementInfo)
{
    ...
    if (player->GetLiquidStatus() != _lastLiquidStatus)
        return;
    ...
    float allowedSpeedRate = player->GetSpeed(UnitMoveType(movementFlag)) * tolerance;
    ...
    // DEBUG
    /* std::string str_speedrates = "Allowed Speedrate: " + std::to_string(allowedSpeedRate) + " - Client Speedrate: " + std::to_string(clientSpeedRate) + " - SpeedHack: " + std::to_string(clientSpeedRate > allowedSpeedRate);
    player->Say(str_speedrates, LANG_UNIVERSAL); */
...
void AntiCheatMgr::CheckForClimbHack(MovementInfo movementInfo, uint32 opcode)
{
        ...
        (!_lastMovementInfo.HasMovementFlag(MOVEMENTFLAG_FALLING) || movementInfo.HasMovementFlag(MOVEMENTFLAG_FALLING)))
    {
        ...
        CreateCheatReport("ClimbHack", "Slope: " + std::to_string(slope));
    }
...
void AntiCheatMgr::CheckForZAxisHack(MovementInfo movementInfo)
{
        ...
        && player->GetPositionZ() >= player->GetFloorZ() + 2.5f)
    {
        ...
        if (_hackCounter >= _hackTolerance)
            CreateCheatReport("ZAxisHack", "LiquidStatus: " + std::to_string(player->GetLiquidStatus()));
    }
...
void AntiCheatMgr::CheckForAntiswimHack(MovementInfo movementInfo)
{
...
void AntiCheatMgr::CheckForIgnoreControlHack()
{
    ...
    if (player->HasUnitState(UNIT_STATE_ROOT) && !player->GetVehicle())
    {
        ...
        if (_hackCounter >= _hackTolerance)
            CreateCheatReport("IgnoreControlHack", "Latency: " + std::to_string(player->GetSession()->GetLatency()));
    }
...
bool AntiCheatMgr::CheckForTransport()
{
    ...
    if (_lastMovementInfo.HasMovementFlag(MOVEMENTFLAG_ONTRANSPORT) && player->GetMapId())
    {
...
void AntiCheatMgr::CreateCheatReport(std::string hacktype, std::string description)
{
    ...
    std::string playerClass = (std::string)GetClassName(player->GetClass(), 0);
    std::string reportLocation = player->GetMapAreaAndZoneString();

    // If the player is mindcontrolled, we want to log the controlling player instead of the controlled player
    if (Player* controller = player->GetControllingPlayer())
    ...
    stmt->setString(8, player->GetMapAreaAndZoneString()); // map_info
